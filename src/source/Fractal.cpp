...
void Fractal::RenderLoop(GLFWwindow* window, Fractal* fractal)
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		{
			glActiveTexture(GL_TEXTURE0 + fractal->shader->textures[0].unitId);
			glBindTexture(GL_TEXTURE_2D, fractal->shader->textures[0].id);
			glActiveTexture(GL_TEXTURE0 + fractal->shader->textures[1].unitId);
			glBindTexture(GL_TEXTURE_2D, fractal->shader->textures[1].id);
		}
...
void Fractal::ImageSequence(GLFWwindow* window, Fractal* fractal)
{
	...
	const static int imageCount = 1000; 
	...
	((Uniform<float>*)fractal->fractalUniforms[6].uniform)->value = 1.f/3.f;
	((Uniform<float>*)fractal->fractalUniforms[6].uniform)->SetShaderValue(false);
	((Uniform<float>*)fractal->fractalUniforms[4].uniform)->value = 94.f;
	...
	fractal->clickPosition.value = { 0.52381, -0.00370371 }; // -0.753948450, -0.0111111403
	...
	for (size_t i = 0; i < imageCount; i++)
	{
		...
		fractal->clickPosition.value = { 0.52381+float(i)/imageCount*0.1, -0.00370371 }; // -0.753948450, -0.0111111403
		fractal->shader->SetUniform(fractal->clickPosition);
...
void Fractal::SaveImage(std::string path)
{
	...
	{
		glBindTexture(GL_TEXTURE_2D, shader->textures[0].id);
	}
